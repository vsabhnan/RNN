...
# Comparison of best and worst models
...
grad = False
run_index += 1
hist_best = run_iter(run_index, arch, activation, opt, grad, seq_length, datasets)
...
grad = False
run_index += 1
hist_worst = run_iter(run_index, arch, activation, opt, grad, seq_length, datasets)

# Plot Training Loss and Epochs for Best and Worst Models
plt.plot(hist_worst.history["loss"], label="Worst Model (RNN)")
plt.plot(hist_best.history["loss"], label="Best Model (LSTM)")
plt.xlabel("Epoch")
plt.ylabel("Training Loss")
plt.title("Training Loss vs. Epoch")
plt.legend()
plt.grid(True)
plt.savefig("results/plots/loss_comparison.png", dpi=300, bbox_inches="tight")
plt.show()
